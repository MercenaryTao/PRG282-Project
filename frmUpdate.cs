...
{   
    public partial class frmUpdate : Form
    {
        ...
        public frmUpdate()
        {
            ...
            //Change the following three string parameters to the file directory as per your system:
                ...
                fileHandler3 = new FileHandler(@"C:\Users\kaira\source\repos\PRG282-Project\bin\Debug\deletedStudents.txt");
            ...
            try
            {
                ...
                if (studentFound)
                {
                    ...
                    if (dataGridView1.SelectedCells.Count > 0)
                    {
                        ...
                        using (StreamWriter sw = new StreamWriter(fileHandler3.StudentFile, true))
                        {
                            sw.WriteLine($"{sID} {name} {surname} {age} {course}");
                        }
                        List<string> allLines = File.ReadAllLines(fileHandler.StudentFile).ToList();
                        allLines.RemoveAt(selectedRowIndex);
                        File.WriteAllLines(fileHandler3.StudentFile, allLines.ToArray());
        ...
        private void btnUndo_Click(object sender, EventArgs e)
        {
            ...
            List<string> deletedLines = File.ReadAllLines(fileHandler3.StudentFile).ToList();
            foreach (string line in deletedLines)
            {
                string[] data = line.Split(' ');
                if (data[0].Trim() == searchID)
                {
                    using (StreamWriter sw = File.AppendText(fileHandler.StudentFile))
                    {
                        sw.WriteLine(line);
                    }
                    deletedLines.Remove(line);
                    deletedLines.Remove(line);
                    File.WriteAllLines(fileHandler3.StudentFile, deletedLines);
        ...
        dataGridView1.Rows.Clear();
            ...
            var lines = File.ReadAllLines(fileHandler.StudentFile);
