    ...
    public partial class frmDashboard : Form
    {
        ...
        public frmDashboard()
        {
            ...
            //Change the following two string pararmeters to the file directory as per your system:
                fileHandler = new FileHandler(@"C:\Users\kaira\source\repos\PRG282-Project\students.txt");
                fileHandler2 = new FileHandler(@"C:\Users\kaira\source\repos\PRG282-Project\dataFile.txt");
        }
        ...
        private void saveToFile_Click(object sender, EventArgs e)///saves to the file
        {
            ...
            try
            {
                ...
                if (!int.TryParse(age, out int oldAge))
                {
                ...
                students.Clear();
                students.Add(student);
                using (StreamWriter sr = new StreamWriter(fileHandler.StudentFile, append: true))
                {
                    foreach (var item in students)
                    {
        ...
        {
            string srchID = stuID.Text.Trim();
            string srchName = stuName.Text.Trim();
            string srchSurname = stuSurname.Text.Trim();
            string srchAge = stuAge.Text.Trim();
            string srchCourse = stuCourse.Text.Trim();
            ...
            try
            {
                foreach (string line in File.ReadLines(fileHandler.StudentFile))
                {
                    ...
                    bool matches =
                        (string.IsNullOrEmpty(srchID) || srchID == sID) &&
                        (string.IsNullOrEmpty(srchName) || srchName == name) &&
                        (string.IsNullOrEmpty(srchSurname) || srchSurname == surname) &&
                        (string.IsNullOrEmpty(srchAge) || srchAge == age) &&
                        (string.IsNullOrEmpty(srchCourse) || srchCourse == course);

                    if (matches)
                    {
                        dataDisplay.Rows.Add(sID, name, surname, age, course);
                    }
                }

                if (dataDisplay.Rows.Count == 0)
                {
                    MessageBox.Show("No students found matching the search criteria.", "Search Result", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
